...
- **사진 없는 변형의 `BODY_COLUMN_1`과 `BODY_COLUMN_2`는 InDesign 텍스트 스레드로 실제 연결되어 있음이 확인되었다**(2026-09-23, 사용자가 InDesign에서 직접 확인: `BODY_COLUMN_1.nextTextFrame` → `BODY_COLUMN_2`, `BODY_COLUMN_2.previousTextFrame` → `BODY_COLUMN_1`). 이 확인을 근거로 아래처럼 데이터 계약을 단순화했다.
...
그래서 **`variant` 필드로 명시적으로 선언**하게 했다. 어느 InDesign 페이지 변형을 쓸지는 데이터를 만드는 쪽이 결정해서 명시하고, 코드는 그 값을 그대로 신뢰한다(추측하지 않는다). 이 결정은 이번 단순화 이후에도 그대로 유지한다.

## 왜 본문은 `bodyColumn1`/`bodyColumn2`가 아니라 단일 `body`인가 (2026-09-23 변경)

처음에는 사진 없는 변형의 2단 본문을 `bodyColumn1`/`bodyColumn2` 두 필드로 명시적으로 받도록 설계했다. 이유는 "본문 하나를 코드가 어디서 잘라 두 프레임에 나눠 넣을지 예측하기 어렵다"는 것이었다 — 프레임 크기·폰트·자간에 따라 정확히 어디서 잘려야 자연스러운지는 코드가 미리 알 수 없기 때문이다.

이후 실제 InDesign에서 `BODY_COLUMN_1`과 `BODY_COLUMN_2`가 **InDesign 자체의 텍스트 스레드(Text Thread)로 이미 연결되어 있음**을 확인했다(`BODY_COLUMN_1.nextTextFrame` = `BODY_COLUMN_2`, `BODY_COLUMN_2.previousTextFrame` = `BODY_COLUMN_1`). 텍스트 스레드로 연결된 프레임은, 앞쪽 프레임(`BODY_COLUMN_1`)에 본문 전체를 채우면 **InDesign이 자체적으로 넘치는 텍스트를 다음 프레임(`BODY_COLUMN_2`)으로 흘려보낸다.** 즉 "어디서 자를지"를 코드가 예측할 필요가 애초에 없다 — 코드는 텍스트를 자르지 않고, InDesign의 텍스트 스레드가 처리한다.

그래서 사진 없는 변형도 `body` 필드 하나만 받고, 그 값을 텍스트 스레드의 시작 프레임인 `BODY_COLUMN_1`에 채워 넣는 것으로 계약을 단순화했다. `bodyColumn1`/`bodyColumn2`로 필드를 나눴던 이전 설계는 더 이상 쓰지 않는다.
...
| `body` | string | **Required** | 본문 전체. 들어가는 프레임은 variant에 따라 다르다 — `WITH_PHOTO`는 `BODY`, `WITHOUT_PHOTO`는 `BODY_COLUMN_1`(텍스트 스레드로 `BODY_COLUMN_2`까지 자동으로 흐름). 아래 매핑표 참고. |
...
추가 필드 없음. `body` 하나만 쓰고 `BODY_COLUMN_1`에 채워 넣으면 텍스트 스레드로 `BODY_COLUMN_2`까지 자동으로 흐른다. `heroImage`는 사용하지 않는다 (이 페이지 변형에는 `HERO_IMAGE` 프레임 자체가 없음 — Inspect Template으로 확인됨).

## JSON 필드 ↔ Script Label 매핑 요약

| JSON 필드 | InDesign Script Label | Object Type | 적용 variant | 비고 |
|---|---|---|---|---|
| `title` | `TITLE` | TextFrame | 공통 | |
| `pointText` | `POINT_TEXT` | TextFrame | 공통 | |
| `body` | `BODY` | TextFrame | `WITH_PHOTO` | |
| `body` | `BODY_COLUMN_1` | TextFrame | `WITHOUT_PHOTO` | `BODY_COLUMN_1`에만 쓴다. `BODY_COLUMN_2`는 텍스트 스레드로 자동 연결되어 있으므로 **직접 쓰지 않는다.** |
| `heroImage` | `HERO_IMAGE` | Rectangle | `WITH_PHOTO` | |

## 이번 작업에서 확정한 것 (2026-09-23)

- 사진 있음/없음 구분: `variant` 필드로 명시 (heroImage 유무로 암묵 추측하지 않음) — 기존 결정 유지
- **본문 계약 단순화**: `bodyColumn1`/`bodyColumn2` 두 필드를 폐기하고, 두 variant 모두 공통으로 `body` 필드 하나를 사용한다. `WITHOUT_PHOTO`에서는 이 값을 `BODY_COLUMN_1`에만 쓰고, `BODY_COLUMN_2`는 InDesign 텍스트 스레드가 자동으로 채운다 (실기로 연결 확인됨)
...
- [sample/opening-page-without-photo.json](../sample/opening-page-without-photo.json) — 2026-09-23, `bodyColumn1`/`bodyColumn2`에서 단일 `body`로 갱신됨
